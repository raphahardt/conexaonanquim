...
class DB {
  ...
  public function __construct($dbname = DJCK_DB_SCHEMA, $dbuser = DJCK_DB_USER, $dbpass = _DJCK_DB_PASSWORD, $dbhost = DJCK_DB_HOST, $charset = 'UTF8') {
    ...
    if (mysqli_connect_error()) {
      // erro de conexão
      Handler::error(mysqli_connect_errno() . ': ' . mysqli_connect_error());
    }
  ...
  public function execute() {
    ...
    if ($instruction == 'SELECT') {
      ...
      if (mysqli_error($this->con)) {
        // erro de conexão
        throw new Exception(mysqli_errno($this->con) . ': ' . mysqli_error($this->con));
      }
      ...
      if (mysqli_error($this->con)) {
        // erro de conexão
        Handler::error(mysqli_errno($this->con) . ': ' . mysqli_error($this->con));
      }
